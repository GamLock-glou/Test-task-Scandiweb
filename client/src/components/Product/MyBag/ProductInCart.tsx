                    ...
                    return <div className='productsCart'>
                        <div className='leftCart'>
                            ...
                            <div>{attributes.map((attribute, key) => {
                                return <AttributeProductInCart key={key} attributeCart={product.attributes} attribute={attribute}/>
                            })}</div>
                        </div>
                        <div className='rightCart'>
                            <div className='buttonsCountProduct'>
                                <div className='buttonCountProduct' onClick={()=>addProductCount(id)}>+</div>
                                <div className='countProducts'>{product.productCount}</div>
                                <div className='buttonCountProduct' onClick={()=>addProductCount(id, -1)} >-</div>
                            </div>
                            <img src={gallery[0]}/>
                        </div>
                    </div>
